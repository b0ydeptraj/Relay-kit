#!/usr/bin/env python3
"""
Python Kit - Analyze projects and generate AI agent skills.
...
def _template_destinations(project_path: str, ai: str, skill_name: str) -> list:
    if ai == "generic":
        return [(Path(project_path) / ".python-kit-prompts" / "skills" / skill_name, "generic")]
...
def _build_skill_content(prompt: str, skill_name: str) -> str:
    ...
    if frontmatter_match:
        ...
        frontmatter = f"name: {skill_name}\ndescription: Generated by Python Kit"
...
def run_generic(prompt: str, project_path: str, skill_name: str, verbose: bool = False) -> int:
    ...
    output_dir = Path(project_path) / ".python-kit-prompts"
...
def create_python_skills(
    ...
    print(f"Python Kit v2.1 - Analyzing project at: {project_path}")
    ...
    if not failed_commands and not asset_errors:
        ...
        if ai == "generic":
            print(f"Prompts saved to: {project_path}/.python-kit-prompts/")
        elif ai == "claude":
...
def main():
    parser = argparse.ArgumentParser(
        description="Python Kit v2.1 - Generate AI agent skills from multiple kits",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python python_kit.py                      # Default python skill set with Claude
  python python_kit.py /path/to/project     # Analyze specific project
  python python_kit.py --kit flutter        # Run Flutter analysis skills
  python python_kit.py --kit antigravity    # Install Antigravity template skills
  python python_kit.py --kit claudekit      # Install ClaudeKit template skills
  python python_kit.py --kit ui-ux          # Install UI/UX Pro Max skill
  python python_kit.py --kit full           # Run all skills (analysis + templates)
  python python_kit.py --skills project-architecture testing-patterns
...
AI Adapters:
  ...
  --ai generic  -> .python-kit-prompts/ (copy-paste for any AI)
