...
def build_report(project_root: Path | str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="relay-package-smoke-") as temp_dir:
        ...
        build = run_command(
            [sys.executable, "-m", "pip", "wheel", str(root), "-w", str(wheelhouse)],
            cwd=root,
        )
        ...
        smoke_bin = str(relay_bin) if relay_bin.exists() else [str(venv_python), "-m", "relay_kit_public_cli"]
        if isinstance(smoke_bin, str):
            cmd_base = [smoke_bin]
        else:
            cmd_base = smoke_bin

        smoke = run_command(cmd_base + ["--list-skills"], cwd=root)
        smoke_doctor = run_command(cmd_base + ["runtime", "doctor", "--help"], cwd=root)
        
        status = "pass" if (smoke.returncode == 0 and "baseline" in smoke.stdout and smoke_doctor.returncode == 0) else "fail"
